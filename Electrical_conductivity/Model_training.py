...
def main():
    ...
    df = pd.read_csv("Electrical_conductivity_features.csv")
    ...
    pd.DataFrame([results]).to_csv("Cond_performance.csv", index=False)
    joblib.dump({"model": xgb_best, "feature_cols": feature_cols, "target_col": target_col}, "Cond_model_xgb.pkl")
