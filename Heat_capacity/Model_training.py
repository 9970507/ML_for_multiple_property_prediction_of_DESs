...
def main():
    target_col = "Heat_capacity, J.mol-1.K-1"
    exp_feature_cols = ["X#1", "X#2", "T, K"]

    df = pd.read_csv("Heat_capacity_features.csv")

    molformer1_cols = sorted([c for c in df.columns if c.startswith("MoLFormer1_")])
    molformer2_cols = sorted([c for c in df.columns if c.startswith("MoLFormer2_")])
    feature_cols = molformer1_cols + molformer2_cols + exp_feature_cols
    ...
    pd.DataFrame([results]).to_csv("Cp_performance.csv", index=False)
    joblib.dump({"model": xgb_best, "feature_cols": feature_cols, "target_col": target_col}, "Cp_model_xgb.pkl")
