...
def main():
    target_col = "Melting_point, K"
    exp_feature_cols = ["X#1", "X#2", "T#1, K", "T#2, K"]

    df = pd.read_csv("Melting_point_features.csv")
    ...
    joblib.dump({"model": xgb_best, "feature_cols": feature_cols, "target_col": target_col}, "MP_model_xgb.pkl")
