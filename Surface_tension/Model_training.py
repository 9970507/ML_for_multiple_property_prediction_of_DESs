...
def main():
    target_col = "Surface_tension, mN/m"
    exp_feature_cols = ["X#1", "X#2", "T, K"]

    df = pd.read_csv("Surface_tension_features.csv")
    ...
    pd.DataFrame([results]).to_csv("ST_performance.csv", index=False)
    joblib.dump({"model": xgb_best, "feature_cols": feature_cols, "target_col": target_col}, "ST_model_xgb.pkl")
