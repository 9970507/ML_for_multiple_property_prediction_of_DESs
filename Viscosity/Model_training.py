...
def main():
    target_col = "lg_Viscosity"
    exp_feature_cols = ["X#1", "X#2", "T, K"]

    df = pd.read_csv("Viscosity_features.csv")

    chemberta1_cols = sorted([c for c in df.columns if c.startswith("ChemBERTa1_")])
    chemberta2_cols = sorted([c for c in df.columns if c.startswith("ChemBERTa2_")])

    desc_cols_1 = sorted([
        c for c in df.columns
        if c.endswith("_1") and not c.startswith("ChemBERTa")
        and c != target_col and c not in exp_feature_cols
    ])
    desc_cols_2 = sorted([
        c for c in df.columns
        if c.endswith("_2") and not c.startswith("ChemBERTa")
        and c != target_col and c not in exp_feature_cols
    ])

    feature_cols = (chemberta1_cols + desc_cols_1) + (chemberta2_cols + desc_cols_2) + exp_feature_cols
    ...
    pd.DataFrame([results]).to_csv("Vis_performance.csv", index=False)
    joblib.dump({"model": xgb_best, "feature_cols": feature_cols, "target_col": target_col}, "Vis_model_xgb.pkl")
